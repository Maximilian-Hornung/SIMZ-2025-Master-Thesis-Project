...
emoji_regex <- paste0(
  "(",
    ...
    "|[\U0001F018-\U0001F270]",     # Various emoji ranges
    "|[\U000024C2-\U0001F251]",     # Enclosed characters
    "|[\U0001F1F2-\U0001F1F4]{1,2}",# Regional flags
...
wall_posts_1_cleaned <- networks_labeled[[2]]$wall_posts_labeled |> 
  mutate(
    # Clean unwanted text patterns
    post_text_clean = str_remove_all(post_text, "\\[.*?\\|.*?\\]"),      # [id123|Name]
    post_text_clean = str_remove_all(post_text_clean, "\\[.*?\\]"),           # [alias]
    post_text_clean = str_remove_all(post_text_clean, "@\\w+"),               # @mentions
    post_text_clean = str_remove_all(post_text_clean, "#\\S+"),               # hashtags
    post_text_clean = str_remove_all(post_text_clean, "http[s]?://\\S+"),     # URLs
    post_text_clean = str_remove_all(post_text_clean, "https?:\\S*"),         # '.https:' stubs
    post_text_clean = str_remove_all(post_text_clean, "vk\\.cc/\\S+"),        # short links

    # Remove emojis and emoticons
    post_text_clean = str_remove_all(post_text_clean, emoji_regex),
    post_text_clean = rm_emoticon(post_text_clean),

    # Remove escaped backslashes and HTML tags
    post_text_clean = str_replace_all(post_text_clean, "\\\\", ""),            # remove backslashes like \Влад
    post_text_clean = str_remove_all(post_text_clean, "<[^>]+>"),              # remove HTML tags like <br>, <b> etc.

    # Normalize whitespace and line breaks
    post_text_clean = str_replace_all(post_text_clean, "\\n", " "),            # replace newlines with space
    post_text_clean = str_squish(post_text_clean)                              # collapse multiple spaces
  )

# check text for issues 
check_text <- textclean::check_text(wall_posts_1_cleaned$post_text)
check_text_clean <- textclean::check_text(wall_posts_1_cleaned$post_text_clean)


wall_posts_1_cleaned$post_text_clean[147]


check_text_clean$elemental_checks



# next steps: map this pipleine to all the wallposts, filter for date and language brokered texts, transltae them to english with deepl 
```
...
```{r}
  ...
  clean_text <- function(text, emoji_regex) {
    text |> 
      ...
      str_remove_all("#\\S+") |>                     # hashtags
      str_remove_all("http[s]?://\\S+") |>          # URLs
      str_remove_all("https?:\\S*") |>               # '.https:' stubs
      str_remove_all("vk\\.cc/\\S+") |>             # short links
      str_remove_all(emoji_regex) |>                 # emoji
      rm_emoticon() |>                               # emoticons
      str_replace_all("\\\\", "") |>                 # remove backslashes like \Влад
      str_remove_all("<[^>]+>") |>                   # HTML tags
      str_replace_all("\\n", " ") |>              # newlines to spaces
      str_remove_all(post_text_clean, "\\[.*?\\|.*?\\]") |> 
      str_remove_all(post_text_clean, "\\[.*?\\]") |> 
      str_remove_all(post_text_clean, "\\[#alias\\|.*?\\|.*?\\]") |> 
      str_squish()                                    # trim and collapse multiple spaces
  }


p <- networks_labeled[[1]]$wall_posts_labeled
...
#meaningfully interact 
  ...
  # lang_brokered_df$post_text_clean[641] |>      str_remove_all(regex(
  #     "(\\(*\\s*(KLICK|LIVETICKER|WEBSEITE|TELEGRAM|CHAT|FILME|Portal|Портал|Russland[- ]?Deutsche|Россия Германия)\\s*\\)*)",
  #     ignore_case = TRUE
  #   )) |>
  # 
  #   # Also catch e.g. "FILME (" or "Portal ("
  #   str_remove_all("\\b\\p{L}{2,}\\s*\\(\\s*\\)?") |>    
  #   str_remove_all("\\(\\s*\\)") 
...
# Set API key
Sys.setenv(DEEPL_API_KEY = "b0488e6b-1c9b-40d8-b1ab-a3a4951347b1:fx")
...
prep_k7 \<- estimateEffect(1:7 \~ cld2, stm_k7, meta = prep\$meta, uncertainty = "Global")

summary(prep_k7)

plot(prep_k7, covariate = "cld2", topics = c(1:7), model = stm_k7, method = "difference", cov.value1 = "de", cov.value2 = "ru", xlab = "More Russian-language posts ← Difference in Topic Prevalence → More German-language posts", main = "Effect of Region on Topic Prevalence", #xlim = c(-0.1, 0.1), labeltype = "custom", custom.labels = c("Topic 1", "Topic 2", "Topic 3", "Topic 4", "Topic 5", "Topic 6", "Topic 7"))

# k9

prep_k9 \<- estimateEffect(1:9 \~ cld2, stm_k9, meta = prep\$meta, uncertainty = "Global")

summary(prep_k9)

plot(prep_k9, covariate = "cld2", topics = c(1:9), model = stm_k9, method = "difference", cov.value1 = "de", cov.value2 = "ru", xlab = "More German-language posts ← Difference in Topic Prevalence → More Russian-language posts", main = "Effect of Region on Topic Prevalence", #xlim = c(-0.1, 0.1), labeltype = "custom", custom.labels = c("Topic 1", "Topic 2", "Topic 3", "Topic 4", "Topic 5", "Topic 6", "Topic 7", "Topic 8", "Topic 9"))
...
plot(prep_k7, covariate = "cld2", topics = c(1:7), model = stm_k7, method = "difference", cov.value1 = "de", cov.value2 = "ru", xlab = "More German-language posts ← Difference in Topic Prevalence → More Russian-language posts", main = "Effect of Region on Topic Prevalence", #xlim = c(-0.1, 0.1), labeltype = "custom", custom.labels = c("Topic 1", "Topic 2", "Topic 3", "Topic 4", "Topic 5", "Topic 6", "Topic 7"))

# k8

prep_k8 \<- estimateEffect(1:8 \~ cld2, stm_k8, meta = prep\$meta, uncertainty = "Global")

summary(prep_k8)

plot(prep_k8, covariate = "cld2", topics = c(1:8), model = stm_k8, method = "difference", cov.value1 = "de", cov.value2 = "ru", xlab = "More German-language posts ← Difference in Topic Prevalence → More Russian-language posts", main = "Effect of Region on Topic Prevalence", #xlim = c(-0.1, 0.1), labeltype = "custom", custom.labels = c("Topic 1", "Topic 2", "Topic 3", "Topic 4", "Topic 5", "Topic 6", "Topic 7", "Topic 8"))

# k10

# prep_k10 \<- estimateEffect(1:10 \~ cld2,
...
# xlab = "More German-language posts ← Difference in Topic Prevalence → More Russian-language posts",

# main = "Effect of Region on Topic Prevalence",
