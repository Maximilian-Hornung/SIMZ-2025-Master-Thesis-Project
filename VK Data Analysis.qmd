...
```{r load custom function for data wrangling}
...
get_language_labels <- function(ego_network_list) {
  ...
  clean_text <- function(text, emoji_regex) {
    text |>
      ...
      str_remove_all("vk\\.cc/\\S+") |>
      str_remove_all(emoji_regex) |>
      rm_emoticon() |>
      str_replace_all("\\\\", "") |>
      str_remove_all("<[^>]+>") |>
      str_replace_all("[\t\r\n]", " ") |>
      str_remove_all("\\([^\\)]*\\)|\\[[^\\]]*\\]") |>
      str_remove_all("[!\\-:]{2,}|[:;]{1,2}\\S*") |>
      str_replace_all("%[A-Fa-f0-9]{2}", "") |>
      str_replace_all("\\s+", " ") |>
      str_squish()
  }
  ...
  # Edge list including ego
  edge_list_with_ego <- ego_network_list[["edge_list"]] |>
    filter(if_any(matches("deactivated"), ~is.na(.))) |>
    select(from, to) |>
    filter(!is.na(from) & !is.na(to) & from != to) |>
    distinct()

  # Edge list excluding ego
  ...
  return(list(
    edge_list_with_ego = edge_list_with_ego,
...
attr_df_manual <- read_csv("manual_lang_labels_100_random.csv", col_names = FALSE) |>
  separate(
    col = 1,
    into = c("network_id", "user_id", "lang_manual"),
    sep = ",",
    extra = "merge"
  ) |>
  mutate(across(everything(), ~ gsub('"', '', .))) |> 
  mutate(
    user_id = trimws(as.character(user_id)),  # remove whitespaces
    network_id = trimws(network_id),
    lang_manual = trimws(lang_manual)
  ) |>
  slice(-1)


attr_df_combined <- attr_df_manual |> 
  mutate(user_id = as.character(user_id)) |>  # ensure type matches
  left_join(
    unique_users |> mutate(to = as.character(to)) |> select(user_id = to, lang),
    by = "user_id"
  ) |>
...
# Analytical Strategy: Only keep largest component/ compute centrality value on full ego network, normalize them with z-score
get_igraph <- function(network_labeled, simplify = TRUE, largest_component = TRUE) {

  # Prepare edge lists
  edge_list_with_ego <- network_labeled[["edge_list_with_ego"]] 
  edge_list_1.5_degree <- network_labeled[["edge_list_1.5_degree"]] 
  attr_list <- network_labeled[["attr_list"]]
  
  # Create igraph objects
  g_ego <- igraph::graph_from_data_frame(edge_list_with_ego, directed = FALSE)
  g <- igraph::graph_from_data_frame(edge_list_1.5_degree, directed = FALSE)

  # Calculate betweeness centralities on full ego network g_ego 
  deg_ego <- igraph::degree(g_ego, normalized = TRUE)


  # Name centralities for matching by vertex name
  names(deg_ego) <-  igraph::V(g_ego)$name

  # Map centralities to g vertices by name
  deg_for_g <- deg_ego[igraph::V(g)$name]


  # Assign raw centralities to g
  igraph::V(g)$degree <- deg_for_g


  # Assign other attributes from attr_list
  idx <- match(igraph::V(g)$name, attr_list$to)
  igraph::V(g)$language <- ifelse(is.na(idx), NA, attr_list$lang[idx])
  igraph::V(g)$sex <- ifelse(is.na(idx), NA, attr_list$sex[idx])
  igraph::V(g)$time_absent <- ifelse(is.na(idx), NA, attr_list$time_absent[idx])
  ...
  if (largest_component) {
    ...
    g <- igraph::induced_subgraph(g, which(comps$membership == largest_comp))
    
    cat(glue::glue(
      "For the Language-core network, we observe {comps$no} connected components, ",
      "of which the largest contains {max(comps$csize)} nodes, and the smallest {min(comps$csize)}.\n"
    ))
  }

  # compute z-scores only on nodes in filtered g
  zscore <- function(x) {
    m <- mean(x, na.rm = TRUE)
    s <- sd(x, na.rm = TRUE)
    (x - m) / s
   ...
   igraph::V(g)$degree_z <- zscore(igraph::V(g)$degree)

  
  # BETWEENNESS & CLOSENESS ON FILTERED G
  bet_ego <- igraph::betweenness(g, directed = FALSE, normalized = TRUE) #normalized betweenness
  clo_ego <- igraph::closeness(g, normalized = TRUE)
  igraph::V(g)$betweenness <- bet_ego
  igraph::V(g)$closeness <- clo_ego
...
igraph_list <- mapply(get_igraph, 
                      network_labeled = networks_labeled, 
                      simplify = TRUE) # convert to igraph
...
graph_plot <- function(igraph, seed = 126, show_legend = FALSE, title_text = NULL, net_type = FALSE) {
  set.seed(seed)
  windowsFonts(Times = windowsFont("Times New Roman"))  # Register Times New Roman as Times
  ...
  p <- ggraph(igraph, layout = 'fr') +
    geom_edge_link(color = "grey80", alpha = 0.7, size = 1) +
    geom_node_point(
      aes(color = language, size = betweenness),
      show.legend = c(size = FALSE, color = show_legend)  # hide betweenness legend
    ) +
    scale_size_continuous(range = c(4, 10)) +  # slightly bigger nodes
    scale_color_manual(
      values = lang_colors,
    ...
    guides(
      color = guide_legend(
        override.aes = list(size = 6)  # bigger points in legend
      )
    ) +
    ...
    theme(
      ...
      legend.key.size = unit(0.7, 'cm'),  # bigger legend key space
  ...
  if (!is.null(title_text)) {
    p <- p + 
      geom_text(
        aes(x = -Inf, y = Inf, label = title_text),
        hjust = -0.1, vjust = 1.2,
        size = 6.5, family = "Times", fontface = "plain",
        inherit.aes = FALSE
      )
  }
  
  if (is.character(net_type) && nchar(net_type) > 0) {
    p <- p +
      geom_text(
        aes(x = -Inf, y = Inf, label = net_type),
        hjust = -0.1, vjust = 2.2,
        size = 6.5, family = "Times", fontface = "plain",
        inherit.aes = FALSE
      )
...
# create list of plots for explorative visualization
# plots_list <- igraph_list |> 
#   imap(function(g, i) {
#     graph_plot(
#       igraph = g,
#       seed = 126,
#       show_legend = TRUE,
#       title_text = paste("Network", i),
#       net_type = attr(g, "network_type")
#     )
#   })
...
igraph_clean <- mapply(get_igraph, 
                      network_labeled= multilingual_networks_cleaned, 
                      simplify = TRUE) # convert to igraph


plots_list_clean <- lapply(igraph_clean, graph_plot)
...
```{r network classification and visulaization I}
 ...
 windowsFonts(Times = windowsFont("Times New Roman")) # set fonds 

# generate plots for typical networks types:
gatekeeper <- graph_plot(igraph_clean[[9]], seed = 132, title_text = "1. Gatekeeper")
bridge <- graph_plot(igraph_clean[[21]], seed = 138, title_text = "2. Language Bridge")
peripheral <- graph_plot(igraph_clean[[2]], seed = 289, title_text = "3. Peripheral Language")
union <- graph_plot(igraph_clean[[14]], seed = 132, title_text = "4. Union")
integration <- graph_plot(igraph_clean[[5]], seed = 120, show_legend = TRUE, title_text = "5. Integration")

# Combine plots in 2 rows: 3 plots on top, 2 on bottom, with shared legend below
...
# Iterate over network list for network visualization in appendix 

plots_list_appendix <- purrr::map2(
  igraph_clean,
  network_types,
  ~ graph_plot(.x, seed = 126, show_legend = FALSE, net_type = .y)
)

# plot singular networks and ass them into the appendix 
plots_list_appendix[[17]]
...
```{r Betweenness Centrality and Hypothesis 1}
#| message: false
#| warning: false
#| echo: false

# functions to extract betweenness centrality of user subset
Betweenness_Bilinguals_get  <- function(igraph){

# Extract Bilingual users and calculate Betweeness
Bilinguals <- V(igraph)$name[which(V(igraph)$language == "Bilingual")]
Bilinguals_betweenness <-  igraph::betweenness(igraph, directed = FALSE, v = Bilinguals)

return(Bilinguals_betweenness)
}

Betweenness_Monolinguals_get  <- function(igraph){
# Extract Monolingual users and calculate Betweeness
Monolinguals <- V(igraph)$name[which(V(igraph)$language %in% c("Russian", "German", "Other"))]
Monolinguals_betweenness <-  igraph::betweenness(igraph, directed = FALSE, v = Monolinguals)

return(Monolinguals_betweenness)
}


Betweenness_Bilinguals_mean <- lapply(igraph_clean, Betweenness_Bilinguals_get) |>
  unlist() |> 
  mean(na.rm = TRUE)

print(Betweenness_Bilinguals_mean)

Betweenness_Monolinguals_mean <- lapply(igraph_clean, Betweenness_Monolinguals_get) |>
  unlist() |> 
  mean(na.rm = TRUE)

print(Betweenness_Monolinguals_mean)
```
...
# load data
non_closure_model_results <- readRDS("non_closure_model_results.rds")
...
full_model_summary <- full_model_results  |>
  ...
  mutate(
    coef = case_match(term,
                      ...
                      "nodecov.degree_z" ~ "Popularity",
                      "absdiff.degree_z" ~ "Degree Assortativity",
    ...
    network_type_cat = if_else(model %in% c("network 1", "network 7", "network 8", "network 9"), "Structural Hole Spanners", "Integrated Networks")
  )
...
full_model_summary |> 
  ...
  labs(
    ...
    fill = "Network Type",  
    caption = "Full Model coefficients plotted across all 21 multilingual networks divided by network categories.\n Reference Category is Bilingual:Bilingual and Degree-based selective mixing is normalized with z-scores. \n Source: Author's calculations applying ERGMs to VK network data."
  )
...
# Prepare network/type specific summary
...
produce_ergm_summary_table <- function(i, 
  ...
  modelsummary(
    ...
    coef_rename = c(
      ...
      "nodecov.degree_z" = "Popularity (normalized)",
      "absdiff.degree_z" = "Degree Assortativity (normalized)",
      "gwesp.fixed.1.3" = "Triadic Closure"
    ),
  stars = TRUE,
  add_columns = tibble("% Change*" = ifelse(
  is.na(coef_compare$perc_change),
  "",
  sprintf("%.1f", coef_compare$perc_change)
)),
  gof_map = gof_map,
  notes = "Estimates in conditional (log-)odds ratios. Standard Error in brackets. *Percentage change from NC to Full Model. Coefficients for “NA” category are not shown. Likelihood ratio tests are performed comparing each model to the model in the previous column, (i.e., LM to NC, and NC to Full). I normalized degree-effects using z-scores. Source: Author's ERGM application to VK network data.",
  fmt = 2,
...
ame_mediation_df_cleaned <- ame_mediation_df |> 
  ...
  mutate(
    ...
    network_type_cat = if_else(network_type == "Gatekeeper", "Structural Hole Spanners", "Integrated Networks")
  )  
...
plot(my_fit[[14]], main = '', family = "serif") #Union

plot(my_fit[[9]], main = '', family = "serif") #Gatekeeper

plot(my_fit[[2]], main = '', family = "serif") #Peripheral Language

plot(my_fit[[5]], main = '', family = "serif") #Integration

plot(my_fit[[21]], main = '', family = "serif") #Language Bridge
...
mcmc_diagnostics <- full_model_results |> 
...
# get combined dataframe with all language posts
...
cat(glue("Among the posts filtered for language-brokered posts:
...
# create df for brokered content to share 
# lang_brokered_EN_publication <- lang_brokered_EN |> 
#   select(post_text_EN, post_text_EN_clean, custom_id = id, post_date)
#   saveRDS(lang_brokered_EN_publication, file = "lang_brokered_EN_publication.rds")
```
...
topic_labels <- labelTopics(stm_k6, n = 20)
...
# estimate parameters for model with k6
...
```{r Topic Modelling appendix}

# create word clouds / excluded from Appendix 
...
for (i in 1:6) {
  ...
  topic_label_df <- bind_rows(topic_label_df, topics)  
}
